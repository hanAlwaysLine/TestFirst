...
<html>
	<head>
		...
		<script type="text/javascript"
			src="<%=basePath %>advertisemanage/adchartEdit.js"></script>
					...
					<tr>
					  ...
					  <td align="left">					   
					    <select id="advertisement_type" name="advertise.advertisement_type">
					    	<option value="21" <c:if test="${advertise.advertisement_type==21}">selected="selected"</c:if>>网站侧边栏广告</option>
					    	<option value="22" <c:if test="${advertise.advertisement_type==22}">selected="selected"</c:if>>网站支付框广告</option>
					    	<option value="23" <c:if test="${advertise.advertisement_type==23}">selected="selected"</c:if>>网站会员频道广告</option>
					    </select>
					  </td>
					</tr>
					<tr>
