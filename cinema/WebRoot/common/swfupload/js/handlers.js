...
function fileQueueError(file, errorCode, message) {
	try {
		/*if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			//alert("上传图片数量过多.\n" + (message === 0 ? "You have reached the upload limit." : "你只能选择 " + (message > 1 ? "最多 " + message + "个文件." : "一个文件.")));
			return;
		}*/
		if( errorCode==SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT){
			alert(file.name+"文件过大,最多不能超过5MB！");
			return;
		}
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		...
		//jQuery('#img_upload_button').hide();
		jQuery('.image_preview').fadeIn();
		jQuery('.success_uploading').attr({'disabled': false});
		//this.startUpload();
