...
<html>
	<head>
		...
		<script type="text/javascript"
			src="<%=basePath%>orderselect/orderformResult.js"></script>
		<!--
			...
			<s:iterator id="order" value="#request.list.objects">
				<tr align="center">
					...
					<td>
						...
						<c:if test="${order.RECEIVE_STATUS=='0'}">
							<a href="javascript:receiveOp('${order.ORDER_NO}')">[未领取]</a>
						</c:if>
						<c:if test="${order.RECEIVE_STATUS=='1'}">
							<label style="color: red;font-size: 12px;">[已领取]</label>
						</c:if>
					</td>
				</tr>
			</s:iterator>
